...
import { useCustomerDashboard } from "@/lib/queries/use-mro";
...
export default function CustomerDetailPage() {
  ...
  const wps = (data.work_packages as J[]) ?? []; const sla = (data.sla as J[]) ?? []; const reports = (data.reports as J[]) ?? [];
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <PageHeader eyebrow={String(c.customer_code)} title={String(c.customer_name)} subtitle={`${String(c.customer_type)} · ${String(c.customer_status)} · ${String(c.payment_terms ?? "")}`}
        actions={<Button size="sm" onClick={() => generateReport.mutate({ customerId: id }, { onSuccess: () => toast({ title: "Report generated" }) })}>{generateReport.isPending ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <FileText className="h-3.5 w-3.5" />} Monthly report</Button>} />
      ...
      <div className="flex-1 overflow-y-auto avir-scroll">
        ...
        <Tabs defaultValue="contracts" className="mt-5">
          ...
          <TabsContent value="contracts"><div className="p-6"><div className="border border-border">
            {contracts.map((k) => (
          ...
          <TabsContent value="reports"><div className="p-6"><div className="border border-border">
            {reports.map((r) => (
              <div key={String(r.id)} className="flex items-center gap-x-4 border-b border-border/60 px-3 py-2.5 last:border-b-0">
                <FileText className="h-3.5 w-3.5 text-label" /><span className="text-[13px] text-foreground">{String(r.report_type).replace(/_/g, " ")}</span>
                <span className="ml-auto font-mono text-[11px] text-hint">{String(r.status)}</span>
