...
const sheetVariants = cva(
  "fixed z-50 gap-4 bg-card shadow-lg transition ease-avir data-[state=open]:duration-panel data-[state=closed]:duration-micro",
  {
    variants: {
      side: {
...
>(({ side = "right", className, children, ...props }, ref) => (
  <SheetPortal>
    ...
    <SheetPrimitive.Content ref={ref} className={cn(sheetVariants({ side }), className)} {...props}>
      {children}
      <SheetPrimitive.Close className="absolute right-4 top-4 text-subtext transition-colors duration-micro hover:text-foreground focus:outline-none">
        <X className="h-4 w-4" />
