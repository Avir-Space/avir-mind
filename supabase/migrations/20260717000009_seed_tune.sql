-- 0109: final seed tuning.
--   - guarantee >=2 AOG tasks on the single designated AOG aircraft (so AOG
--     lands at 1-2% of tasks while staying on ONE aircraft)
--   - emit an 'assigned' event whenever a task is seeded with an assignee
--   - raise comment frequency, so task_events comfortably reaches ~600
-- Everything else is identical to 0108's seed_avir_demo.
...
begin
  ...
  for ac in
    ...
    from public.aircraft a join public.aircraft_state s on s.aircraft_id=a.id where a.org_id=v_org_id
  loop
    ...
    for i in 1..v_count loop
      ...
      -- AOG concentrates on the one designated aircraft; force its first two tasks
      -- to be active AOG so AOG lands at ~1-2% of tasks on a single tail.
      v_aog := false;
      ...
      values (v_org_id,ac.id,v_title,v_why,v_parent,v_sub,v_status,v_risk,v_blocking,v_aog,v_station,v_facility,v_due,v_started,
      ...
      if v_assignee is not null then
        insert into public.task_events (org_id,task_id,actor_user_id,event_type,event_payload,created_at_utc)
        values (v_org_id,v_task_id,p_user_id,'assigned',jsonb_build_object('to',v_assignee),v_created+interval '1 hour');
      end if;

      if v_status <> 'queued' then
        insert into public.task_events (org_id,task_id,actor_user_id,event_type,event_payload,created_at_utc)
        values (v_org_id,v_task_id,p_user_id,'status_change',
          jsonb_build_object('from','queued','to',case when v_status='done' then 'in_progress' else v_status end),v_created+interval '2 hours');
      end if;
      ...
      if v_status in ('in_progress','done') and random() < 0.22 then
        ...
        values (v_org_id,v_task_id,p_user_id,(floor(random()*8)+1)*30,'Work performed per task scope.',(v_created+interval '6 hours')::date,v_created+interval '6 hours');
        insert into public.task_events (org_id,task_id,actor_user_id,event_type,event_payload,created_at_utc)
  ...
  insert into public.audit_events (org_id,actor_user_id,entity_type,entity_id,event_type,event_payload)
  values (v_org_id,p_user_id,'org',v_org_id,'tasks.seeded',jsonb_build_object('task_count',(select count(*) from public.tasks where org_id=v_org_id)));
