-- 0107: extend seed_avir_demo to also seed realistic task data.
--
-- Full CREATE OR REPLACE (the aircraft block is preserved verbatim from 0007;
-- the task-generation block is appended). Idempotent via the existing guard.
--
-- Distributions (per the Phase 1 spec):
--   ~200 tasks across 24 aircraft (uneven: some 2-3, some 15+)
--   status:  queued 25% / in_progress 17% / blocked 7% / monitoring 12% / done 39%
--   risk:    high 8% / medium 35% / low 57%
--   dispatch_blocking: ~11% of ACTIVE tasks   aog: ~2% of tasks
--   sources: amos 35 / trax 20 / sap 15 / fr 15 / avir 15
--   plus ~600 task_events, ~35 acknowledgements, ~25 work logs
...
declare
  ...
  v_comments  text[] := array[
    'Parts ordered, ETA 2 days.','Coordinating with line maintenance.','Awaiting engineering disposition.',
...
begin
  ...
  values (v_org_id, 'Fleet 1 — Narrowbody', 'A320 / B737 family') returning id into v_fleet_id;
  v_fleet_ids[1] := v_fleet_id;
  insert into public.fleets (org_id, name, aircraft_type_focus)
  values (v_org_id, 'Fleet 2 — Regional', 'E175 / CRJ-900') returning id into v_fleet_id;
  v_fleet_ids[2] := v_fleet_id;
  insert into public.fleets (org_id, name, aircraft_type_focus)
  values (v_org_id, 'Fleet 3 — Business Jets', 'G650 / Falcon 7X / Global 7500') returning id into v_fleet_id;
  ...
  -- Task seeding
  -- ═══════════════════════════════════════════════════════════════════════════
  select array_agg(parent_type order by parent_type, sort_rank),
         array_agg(sub_type order by parent_type, sort_rank),
         array_agg(display_name order by parent_type, sort_rank)
    into v_parents, v_subs, v_displays
  from public.task_type_catalog where active;
  v_catalog_n := array_length(v_parents, 1);

  for ac in
    select a.id, a.tail_number, a.aircraft_type, a.base_station,
           s.current_station, s.next_event_type
    from public.aircraft a
    join public.aircraft_state s on s.aircraft_id = a.id
    where a.org_id = v_org_id
  loop
    v_count := 3 + floor(random() * 6);                    -- 3..8 baseline
    if random() < 0.18 then v_count := v_count + 6 + floor(random() * 10); end if; -- heavy tails
    if random() < 0.15 then v_count := 2 + floor(random() * 2); end if;            -- light tails
    v_ac_is_aog := (ac.next_event_type = 'AOG Recovery');

    for i in 1..v_count loop
      v_k := 1 + floor(random() * v_catalog_n);
      v_parent := v_parents[v_k];
      v_sub := v_subs[v_k];
      v_display := v_displays[v_k];

      v_rnd := random();
      v_status := case
        when v_rnd < 0.25 then 'queued'
        when v_rnd < 0.42 then 'in_progress'
        when v_rnd < 0.49 then 'blocked'
        when v_rnd < 0.61 then 'monitoring'
        else 'done' end;
      ...
      v_blocking := v_active and random() < 0.11;

      v_aog := false;
      if v_ac_is_aog and i = 1 then
        v_aog := true; v_status := 'in_progress'; v_active := true; v_risk := 'high'; v_blocking := true;
      elsif v_active and random() < 0.015 then
        v_aog := true; v_blocking := true; v_risk := 'high';
      end if;

      v_rnd := random();
      v_src := case
        when v_rnd < 0.35 then 'amos'
        when v_rnd < 0.55 then 'trax'
        when v_rnd < 0.70 then 'sap'
        when v_rnd < 0.85 then 'fr'
        else 'avir' end;
      ...
      v_started := case when v_status in ('in_progress','monitoring','blocked')
                        then v_created + make_interval(hours => floor(random() * 24)::int) else null end;
      ...
      v_why := case v_parent
        ...
        else 'Ground event reported at station requiring follow-up.'
      end;

      v_rank := v_rank + 1;

      insert into public.tasks (
        org_id, aircraft_id, title, why_summary, parent_type, sub_type, status, risk_band,
        dispatch_blocking, aog, station_code, facility, due_at_utc, started_at_utc,
        assignee_user_id, reporter_user_id, board_rank, created_at_utc, updated_at_utc
      ) values (
        v_org_id, ac.id, v_title, v_why, v_parent, v_sub, v_status, v_risk,
        v_blocking, v_aog, v_station, v_facility, v_due, v_started,
        v_assignee, p_user_id, v_rank, v_created, v_created
      ...
      -- task_created (audit thesis: every task begins with an event)
      insert into public.task_events (org_id, task_id, actor_user_id, event_type, event_payload, created_at_utc)
      values (v_org_id, v_task_id, p_user_id, 'task_created',
              jsonb_build_object('title', v_title, 'source_system', v_src), v_created);

      -- status transition history
      if v_status <> 'queued' then
        insert into public.task_events (org_id, task_id, actor_user_id, event_type, event_payload, created_at_utc)
        values (v_org_id, v_task_id, p_user_id, 'status_change',
                jsonb_build_object('from', 'queued', 'to', case when v_status = 'done' then 'in_progress' else v_status end),
                v_created + interval '2 hours');
      end if;
      if v_status = 'done' then
        insert into public.task_events (org_id, task_id, actor_user_id, event_type, event_payload, created_at_utc)
        values (v_org_id, v_task_id, p_user_id, 'status_change',
                jsonb_build_object('from', 'in_progress', 'to', 'done'), v_created + interval '1 day');
      end if;

      -- occasional comment
      if random() < 0.45 then
        insert into public.task_events (org_id, task_id, actor_user_id, event_type, body, created_at_utc)
        values (v_org_id, v_task_id, p_user_id, 'comment',
                v_comments[1 + floor(random() * array_length(v_comments, 1))], v_created + interval '5 hours');
      end if;

      -- acknowledgement on some active tasks
      if v_active and random() < 0.30 then
        ...
        values (v_org_id, v_task_id, p_user_id, 'acknowledged', v_created + interval '3 hours');
      end if;

      -- work log on some in-progress / done tasks
      if v_status in ('in_progress','done') and random() < 0.22 then
        ...
        values (v_org_id, v_task_id, p_user_id, (floor(random() * 8) + 1) * 30,
                'Work performed per task scope.', (v_created + interval '6 hours')::date, v_created + interval '6 hours');
        insert into public.task_events (org_id, task_id, actor_user_id, event_type, event_payload, created_at_utc)
        values (v_org_id, v_task_id, p_user_id, 'work_logged',
                jsonb_build_object('minutes', (floor(random() * 8) + 1) * 30), v_created + interval '6 hours');
      end if;
    end loop;
  end loop;

  insert into public.audit_events (org_id, actor_user_id, entity_type, entity_id, event_type, event_payload)
  values (v_org_id, p_user_id, 'org', v_org_id, 'tasks.seeded',
          jsonb_build_object('task_count', (select count(*) from public.tasks where org_id = v_org_id)));
