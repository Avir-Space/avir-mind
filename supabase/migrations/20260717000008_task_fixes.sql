-- 0108: forward-fix for two issues found during live verification.
--   (a) get_command_center_queue: recent_events subquery ordered by a jsonb
--       column alias instead of the timestamp column, which errored the RPC.
--   (b) seed_avir_demo: AOG was scattered across ~3 aircraft via a random
--       branch; it must concentrate on the single designated AOG aircraft.
--       Also raised comment frequency so task_events lands ~600+.

-- ── (a) get_command_center_queue (corrected recent_events subquery) ───────────
    ...
    'queue', coalesce((
      select jsonb_agg(jsonb_build_object(
        ...
        'recent_events', coalesce((
          select jsonb_agg(sub.ev order by sub.created_at_utc desc)
          from (
            ...
            order by e.created_at_utc desc limit 3
          ) sub), '[]'::jsonb)
        ) order by f.rn)
...
-- ── (b) seed_avir_demo (AOG concentrated on the designated aircraft) ──────────
...
begin
  ...
  loop
    ...
    values (v_ac_id, r.state, r.source, r.conf, r.cur_station,
  ...
  for ac in
    ...
    from public.aircraft a join public.aircraft_state s on s.aircraft_id = a.id
    where a.org_id = v_org_id
  loop
    v_count := 3 + floor(random() * 6);
    if random() < 0.18 then v_count := v_count + 6 + floor(random() * 10); end if;
    if random() < 0.15 then v_count := 2 + floor(random() * 2); end if;
    ...
    for i in 1..v_count loop
      ...
      -- AOG concentrates on the single designated AOG aircraft only.
      v_aog := false;
      if v_ac_is_aog and v_active and random() < 0.5 then
        v_aog := true;
        v_status := case when v_status = 'queued' then 'in_progress' else v_status end;
        v_risk := 'high'; v_blocking := true;
      end if;

      v_rnd := random();
      v_src := case when v_rnd < 0.35 then 'amos' when v_rnd < 0.55 then 'trax'
        when v_rnd < 0.70 then 'sap' when v_rnd < 0.85 then 'fr' else 'avir' end;
      ...
      insert into public.tasks (org_id, aircraft_id, title, why_summary, parent_type, sub_type, status, risk_band,
        dispatch_blocking, aog, station_code, facility, due_at_utc, started_at_utc, assignee_user_id, reporter_user_id,
        board_rank, created_at_utc, updated_at_utc)
      values (v_org_id, ac.id, v_title, v_why, v_parent, v_sub, v_status, v_risk, v_blocking, v_aog, v_station,
        v_facility, v_due, v_started, v_assignee, p_user_id, v_rank, v_created, v_created)
      ...
      values (v_org_id, v_task_id, p_user_id, 'task_created', jsonb_build_object('title', v_title, 'source_system', v_src), v_created);
      ...
      if v_status = 'done' then
        insert into public.task_events (org_id, task_id, actor_user_id, event_type, event_payload, created_at_utc)
        values (v_org_id, v_task_id, p_user_id, 'status_change', jsonb_build_object('from', 'in_progress', 'to', 'done'), v_created + interval '1 day');
      end if;

      if random() < 0.60 then
        insert into public.task_events (org_id, task_id, actor_user_id, event_type, body, created_at_utc)
        values (v_org_id, v_task_id, p_user_id, 'comment', v_comments[1 + floor(random() * array_length(v_comments, 1))], v_created + interval '5 hours');
      end if;
      ...
      if v_status in ('in_progress','done') and random() < 0.22 then
        ...
        values (v_org_id, v_task_id, p_user_id, (floor(random() * 8) + 1) * 30, 'Work performed per task scope.',
                (v_created + interval '6 hours')::date, v_created + interval '6 hours');
        insert into public.task_events (org_id, task_id, actor_user_id, event_type, event_payload, created_at_utc)
        values (v_org_id, v_task_id, p_user_id, 'work_logged', jsonb_build_object('minutes', (floor(random() * 8) + 1) * 30), v_created + interval '6 hours');
      end if;
    end loop;
  end loop;
